...
def route_intent(message: str) -> dict[str, Any]:
    ...
    if any(
        ...
        if "error" in msg_lower or "bug" in msg_lower:
            intent[
                "system_hint"
            ] += " Provide an isolated root-cause hypothesis followed by an explicit step-by-step patch path."
        return intent
    ...
    intent["system_hint"] = (
        "Provide an elite, highly informative, balanced, and contextually granular response."
    )
    return intent
